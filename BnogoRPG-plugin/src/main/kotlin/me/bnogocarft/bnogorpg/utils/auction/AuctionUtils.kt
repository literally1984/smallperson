...
fun getAuctionType(item: ItemStack): AuctionType {
    if (item.hasItemMeta()) {
        ...
        try {
            bItem = BItem(item)
        } catch (ignore: IllegalParameterException) {
        }
        if (bItem != null) {
...
fun createAuctionItemFor(auc: Auction): ItemStack {
    val item = auc.itemStack.clone()
    lateinit var meta: ItemMeta
    lateinit var lore: ArrayList<String>

    if (item.hasItemMeta()) {
        meta = auc.itemStack.itemMeta.clone()
    ...
    lore.add("")
    var diff = auc.endTimeUnix - System.currentTimeMillis() / 1000
    var days: Int = 0
    var hours: Int = 0
    var minutes: Int = 0
    var seconds: Int = 0
...
fun bidOnAuction(name: String, auc: Auction, amount: Double, online: Boolean) {
    if (auc.currentBidder != null) {
        ...
        mSender.messages.add(
            "${ChatColor.RED}(" +
                    "${
                        if (auc.itemStack.hasItemMeta())
                            auc.itemStack.itemMeta.displayName
                        else
                            auc.itemStack.type.toString()
                    })"
        )
        mSender.sendMessage()
    }
